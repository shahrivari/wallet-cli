...
public class KeefApi {
  ...
  public static GrpcClient init() {
    ...
    if (config.hasPath("net.type") && "mainnet".equalsIgnoreCase(config.getString("net.type"))) {
      KeefApi.setAddressPreFixByte(CommonConstant.ADD_PRE_FIX_BYTE_MAINNET);
    } else {
      KeefApi.setAddressPreFixByte(CommonConstant.ADD_PRE_FIX_BYTE_TESTNET);
    }
  ...
  public static void setAddressPreFixByte(byte addressPreFixByte) {
    KeefApi.addressPreFixByte = addressPreFixByte;
  }
  ...
  public static WalletFile CreateWalletFile(byte[] password, byte[] priKey) throws CipherException {
    ...
    walletFile = Wallet.createUltraLight(password, ecKey);
    return walletFile;
  }
   ...
   */
  public KeefApi(char[] password, WalletFile walletFile) {
  ...
  private static WalletFile loadWalletFile(File wallet) throws IOException {
    if (wallet == null) {
  ...
  public static KeefApi loadWalletFromKeystore(char[] password, File file) throws IOException {
    WalletFile walletFile = loadWalletFile(file);
    KeefApi walletApi = new KeefApi(password, walletFile);
    return walletApi;
  }
  ...
  private void showTransactionAfterSign(Transaction transaction)
    ...
    if (transaction.getRawData().getContract(0).getType() == ContractType.CreateSmartContract) {
      ...
      System.out.println(
          "Your smart contract address will be: " + KeefApi.encode58Check(contractAddress));
    }
  }
  ...
  public static VoteWitnessContract createVoteWitnessContract(byte[] owner,
    ...
    for (String addressBase58 : witness.keySet()) {
      ...
      byte[] address = KeefApi.decodeFromBase58Check(addressBase58);
      if (address == null) {
        continue;
      }
  ...
  public static boolean addressValid(byte[] address) {
    ...
    if (preFixbyte != KeefApi.getAddressPreFixByte()) {
      System.out.println(
          "Warning: Address need prefix with "
              + KeefApi.getAddressPreFixByte()
              + " but "
              + preFixbyte
              + " !!");
      return false;
  ...
  public static byte[] getAddressFromPrivateKey(byte[] privKey){
    ECKey eckey = ECKey.fromPrivate(privKey);
    return eckey.getAddress();
  }

  public static String getAddressStringFromPrivateKey(byte[] privKey){
    return KeefApi.encode58Check(getAddressFromPrivateKey(privKey));
  }
  ...
  private static byte[] replaceLibraryAddress(String code, String libraryAddressPair,
    ...
    for (int i = 0; i < libraryAddressList.length; i++) {
      ...
      try {
        libraryAddressHex = (new String(Hex.encode(KeefApi.decodeFromBase58Check(addr)),
  ...
  private Permission json2Permission(JSONObject json) {
    ...
    if (json.containsKey("keys")) {
      ...
      for (int i = 0; i < keys.size(); i++) {
        ...
        keyBuilder.setAddress(ByteString.copyFrom(KeefApi.decode58Check(address)));
  ...
  public static boolean sendShieldedCoin(PrivateParameters privateParameters, KeefApi wallet)
      throws CipherException, IOException, CancelException {
  ...
  public static boolean sendShieldedCoinWithoutAsk(PrivateParametersWithoutAsk privateParameters,
      byte[] ask, KeefApi wallet) throws CipherException, IOException, CancelException {
    TransactionExtention transactionExtention =
