...
@Slf4j
public class KeefApiWrapper {

  private KeefApi wallet;


  public File importWalletToTemp(char[] password, byte[] priKey) throws CipherException, IOException {
    if (!KeefApi.passwordValid(password)) {
      return null;
    }
    if (!KeefApi.priKeyValid(priKey)) {
      return null;
    }

    byte[] passwd = StringUtils.char2Byte(password);

    WalletFile walletFile = KeefApi.CreateWalletFile(passwd, priKey);
    StringUtils.clear(passwd);

    File tempFile = File.createTempFile("keef", "tmp");
    tempFile.deleteOnExit();
    WalletUtils.writeWalletToFile(walletFile, tempFile);
  ...
  public static KeefApiWrapper createFromPrivateKey(byte[] privKey) throws CipherException, IOException {
    KeefApiWrapper wrapper = new KeefApiWrapper();
    wrapper.login(privKey);
    return wrapper;
  }

  private static String chars = "QWERTYUIOPASDFGHJLZXCVBNMqwertyuiopasdfghjklzxcvbnm123456789!@#$%^&*)(_+";
  public boolean login(byte[] privKey) throws IOException, CipherException {
    char[] password = RandomStringUtils.random(20, chars).toCharArray();
    logout();
    File walletFile = importWalletToTemp(password, privKey);
    wallet = KeefApi.loadWalletFromKeystore(password, walletFile);
    if (wallet == null) {
...
//      System.out.println("Warning: GetAddress failed,  Please login first !!");
    ...
    return KeefApi.encode58Check(wallet.getAddress());
  }
  ...
  public AddressPrKeyPairMessage generateAddress() {
    ...
    return KeefApi.generateAddress();
  }
  ...
  public Block getBlock(long blockNum) {
    return KeefApi.getBlock(blockNum);
  }

  public long getTransactionCountByBlockNum(long blockNum) {
    return KeefApi.getTransactionCountByBlockNum(blockNum);
  }

  public BlockExtention getBlock2(long blockNum) {
    return KeefApi.getBlock2(blockNum);
  }
  ...
  public Optional<AssetIssueList> getAssetIssueList() {
    try {
      return KeefApi.getAssetIssueList();
    } catch (Exception ex) {
      ex.printStackTrace();
      return Optional.empty();
    }
  ...
  public Optional<AssetIssueList> getAssetIssueList(long offset, long limit) {
    try {
      return KeefApi.getAssetIssueList(offset, limit);
    } catch (Exception ex) {
      ex.printStackTrace();
      return Optional.empty();
    }
  ...
  public AssetIssueContract getAssetIssueByName(String assetName) {
    return KeefApi.getAssetIssueByName(assetName);
  }

  public Optional<AssetIssueList> getAssetIssueListByName(String assetName) {
    try {
      return KeefApi.getAssetIssueListByName(assetName);
    } catch (Exception ex) {
      ex.printStackTrace();
      return Optional.empty();
    }
  ...
  public AssetIssueContract getAssetIssueById(String assetId) {
    return KeefApi.getAssetIssueById(assetId);
  }

  public Optional<ProposalList> getProposalListPaginated(long offset, long limit) {
    try {
      return KeefApi.getProposalListPaginated(offset, limit);
    } catch (Exception ex) {
      ex.printStackTrace();
      return Optional.empty();
    }
  ...
  public Optional<ExchangeList> getExchangeListPaginated(long offset, long limit) {
    try {
      return KeefApi.getExchangeListPaginated(offset, limit);
    } catch (Exception ex) {
      ex.printStackTrace();
      return Optional.empty();
    }
  ...
  public NumberMessage getTotalTransaction() {
    return KeefApi.getTotalTransaction();
  }

  public NumberMessage getNextMaintenanceTime() {
    return KeefApi.getNextMaintenanceTime();
  }
  ...
  public Optional<ChainParameters> getChainParameters() {
    try {
      return KeefApi.getChainParameters();
    } catch (Exception ex) {
      ex.printStackTrace();
      return Optional.empty();
    }
  ...
  public boolean sendShieldedCoin(String fromAddress, long fromAmount, List<Long> shieldedInputList,
    ...
    if (!StringUtil.isNullOrEmpty(fromAddress) && fromAmount > 0) {
      byte[] from = KeefApi.decodeFromBase58Check(fromAddress);
      if (from == null) {
        return false;
      }
    ...
    if (!StringUtil.isNullOrEmpty(toAddress)) {
      byte[] to = KeefApi.decodeFromBase58Check(toAddress);
      if (to == null) {
        return false;
      }
    ...
    if (shieldedInputList.size() > 0) {
      ...
      Optional<IncrementalMerkleVoucherInfo> merkleVoucherInfo =
          KeefApi.GetMerkleTreeVoucherInfo(request.build(), true);
    ...
    return KeefApi.sendShieldedCoin(builder.build(), wallet);
  }
  ...
  public boolean sendShieldedCoinWithoutAsk(String fromAddress, long fromAmount,
    ...
    return KeefApi.sendShieldedCoinWithoutAsk(builder.build(), ask, wallet);
  }
  ...
  public boolean scanNoteByIvk(final String ivk, long start, long end) {
    ...
    Optional<DecryptNotes> decryptNotes = KeefApi.scanNoteByIvk(ivkDecryptParameters, true);
...
//            System.out.println("complete.");
  ...
  public boolean scanAndMarkNoteByAddress(final String shieldedAddress, long start, long end) {
    ...
    try {
      ...
      Optional<DecryptNotesMarked> decryptNotes = KeefApi.scanAndMarkNoteByIvk(builder.build());
      if (decryptNotes.isPresent()) {
        System.out.println(Utils.formatMessageString(decryptNotes.get()));
...
//                }
  ...
  public boolean scanShieldedNoteByovk(final String shieldedAddress, long start, long end) {
    ...
    Optional<DecryptNotes> decryptNotes = KeefApi.scanNoteByOvk(ovkDecryptParameters, true);
    if (!decryptNotes.isPresent()) {
...
//            }
  ...
  public Optional<ShieldedAddressInfo> getNewShieldedAddress() {
    ...
    try {
      Optional<BytesMessage> sk = KeefApi.getSpendingKey();
      Optional<DiversifierMessage> d = KeefApi.getDiversifier();

      Optional<ExpandedSpendingKeyMessage> expandedSpendingKeyMessage = KeefApi
      ...
      Optional<BytesMessage> ak = KeefApi.getAkFromAsk(askBuilder.build());

      BytesMessage.Builder nskBuilder = BytesMessage.newBuilder();
      nskBuilder.setValue(expandedSpendingKeyMessage.get().getNsk());
      Optional<BytesMessage> nk = KeefApi.getNkFromNsk(nskBuilder.build());
      ...
      Optional<IncomingViewingKeyMessage> ivk = KeefApi
      ...
      Optional<PaymentAddressMessage> addressMessage = KeefApi
...
//            System.out.println("ovk " + ByteArray.toHexString(expandedSpendingKeyMessage.get().getOvk().toByteArray()));
  ...
  public String getShieldedNulltifier(long index) {
    ...
    try {
      ...
      Optional<BytesMessage> nullifier = KeefApi.createShieldedNullifier(builder.build());
  ...
  public byte[] getRcm() {
    return KeefApi.getRcm().get().getValue().toByteArray();
  }
  ...
  public NumberMessage getReward(byte[] ownerAddress) {
    return KeefApi.getReward(ownerAddress);
  }

  public NumberMessage getBrokerage(byte[] ownerAddress) {
    return KeefApi.getBrokerage(ownerAddress);
  }
  ...
  public boolean scanShieldedTRC20NoteByIvk(byte[] address, final String ivk,
    ...
    IvkDecryptTRC20Parameters.Builder builder = IvkDecryptTRC20Parameters
        .newBuilder();
    builder.setStartBlockIndex(start)
           .setEndBlockIndex(end)
    ...
    IvkDecryptTRC20Parameters parameters = builder.build();

    Optional<DecryptNotesTRC20> notes = KeefApi.scanShieldedTRC20NoteByIvk(
        parameters, true);
    if (!notes.isPresent()) {
      return false;
    }
    if (notes.get().getNoteTxsList().size() > 0) {
      ...
      if (ShieldedTRC20Wrapper.getInstance().ifShieldedTRC20WalletLoaded()
          && ByteUtil.equals(address, KeefApi.decodeFromBase58Check(
  ...
  public boolean scanShieldedTRC20NoteByOvk(final String ovk, long start, long end,
    ...
    OvkDecryptTRC20Parameters.Builder builder = OvkDecryptTRC20Parameters.newBuilder();
    builder.setStartBlockIndex(start)
           .setEndBlockIndex(end)
    ...
    OvkDecryptTRC20Parameters parameters = builder.build();
    Optional<DecryptNotesTRC20> notes = KeefApi.scanShieldedTRC20NoteByOvk(parameters, true);
    if (!notes.isPresent()) {
      return false;
    }
    if (notes.get().getNoteTxsList().size() > 0) {
      BigInteger scalingFactor;
      if (ShieldedTRC20Wrapper.getInstance().ifShieldedTRC20WalletLoaded()
          && ByteUtil.equals(contractAddress, KeefApi.decodeFromBase58Check(
          ShieldedTRC20Wrapper.getInstance().getShieldedTRC20ContractAddress()))) {
        scalingFactor = ShieldedTRC20Wrapper.getInstance().getScalingFactor();
      } else {
        try {
      ...
      for(DecryptNotesTRC20.NoteTx noteTx : notes.get().getNoteTxsList()) {
        ...
        if (noteTx.hasNote()) {
          ...
          try {
            String toAddress =
                KeefApi.encode58Check(noteTx.getTransparentToAddress().toByteArray());
  ...
  public boolean sendShieldedTRC20Coin(int shieldedContractType, BigInteger fromAmount,
    ...
    byte[] shieldedContractAddressBytes = KeefApi.decodeFromBase58Check(shieldedContractAddress);
    ...
    if (shieldedOutputList.size() > 0) {
      for (int i = 0; i < shieldedOutputList.size(); i++) {
        Note note = shieldedOutputList.get(i);
    ...
    ShieldedTRC20Parameters parameters =
        KeefApi.createShieldedContractParameters(builder.build());
    if (parameters == null) {
  ...
  public boolean sendShieldedTRC20CoinWithoutAsk(int shieldedContractType, BigInteger fromAmount,
    ...
    ShieldedTRC20Parameters parameters =
        KeefApi.createShieldedContractParametersWithoutAsk(builder.build(), ask);
    if (parameters == null) {
  ...
  public String getRootAndPath(String address, long position) {
    byte[] shieldedContractAddress = KeefApi.decodeFromBase58Check(address);
  ...
  public boolean setAllowance(String contractAddress, String shieldedContractAddress,
    ...
    byte[] contractAddressBytes = KeefApi.decodeFromBase58Check(contractAddress);
    byte[] shieldedContractAddressBytes = KeefApi.decodeFromBase58Check(shieldedContractAddress);
  ...
  public static Optional<TransactionInfoList> getTransactionInfoByBlockNum(long blockNum) {
    return KeefApi.getTransactionInfoByBlockNum(blockNum);
  }
